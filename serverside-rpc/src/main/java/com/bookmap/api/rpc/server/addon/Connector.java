...
public class Connector {
  ...
  private final Map<String, LiveConnectionListener>
      liveSubscriptionListenersByAlias = new ConcurrentHashMap<>();
  ...
  public boolean isConnected() { return connectionListener.isConnected(); }

  public boolean isConnectedToProvider(String providerName) {
    System.out.println(broadcasterConsumer.getSubscriptionProviders());
    return broadcasterConsumer.getSubscriptionProviders().contains(
        providerName);
  }

  public void subscribeToLiveData(String generatorName, EventLoop eventLoop,
                                  String providerName,
                                  boolean doesRequireFiltering) {
    if (isConnected()) {
      Optional<GeneratorInfo> generatorInfoOptional =
          getGeneratorInfo(generatorName, providerName);
      generatorInfoOptional.ifPresent(generatorInfo -> {
        LiveConnectionListener subscriptionListener =
            liveSubscriptionListenersByAlias.computeIfAbsent(
                generatorName,
                listener
                -> new LiveConnectionListener(generatorInfo,
                                              broadcasterConsumer));

        FilterListener filterListener =
            new FilterListener(doesRequireFiltering);

        // Creating a listener for the events themselves.
        EventListener eventListener =
            new EventListener(eventLoop, generatorName, filterListener);

        // Trying to subscribe for live events.
        // Broadcasting will notify us of a successful subscription through the
        // LiveConnectionListener.

        broadcasterConsumer.setListenersForGenerator(
            providerName, generatorName, filterListener,
            new SettingsListener(eventLoop, generatorName));
        broadcasterConsumer.subscribeToLiveData(
            providerName, generatorInfo.getGeneratorName(), Event.class,
            eventListener, subscriptionListener);
      });
    }
  }

  private Optional<GeneratorInfo> getGeneratorInfo(String generatorName,
                                                   String providerName) {
    GeneratorInfo generatorInfo = null;
    List<GeneratorInfo> generatorsInfo =
        broadcasterConsumer.getGeneratorsInfo(providerName);
    for (GeneratorInfo generator : generatorsInfo) {
      // In AbsorptionIndicator, generators have the names of the aliases they
      // work with. Therefore, according to the alias, we will get a suitable
      // generator for us.
  ...
  public void finish() { service.shutdownNow(); }

  public boolean isSubscribeToLive(String alias) {
    LiveConnectionListener listener =
        liveSubscriptionListenersByAlias.get(alias);
