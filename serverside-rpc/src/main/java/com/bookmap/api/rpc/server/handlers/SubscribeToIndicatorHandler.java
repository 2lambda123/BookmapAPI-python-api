...
public class SubscribeToIndicatorHandler
    implements Handler<SubscribeToIndicatorEvent> {
  private final EventLoop eventLoop;
  private final Connector connector;
  private final ExecutorService service;

  public SubscribeToIndicatorHandler(EventLoop eventLoop, Connector connector,
  ...
  public void handle(SubscribeToIndicatorEvent event) {
    if (event.alias == null) {
      RpcLogger.info("Generator name is null, connecting to provider " +
                     event.addonName);
      ...
      service.execute(() -> {
        try {
          ...
          throw new RuntimeException(
              String.format("Exception during subscribing to live data for %s,"
                                + " indicator name: %s",
                            event.alias, event.addonName),
              e);
        }
        System.out.println(connector.getBroadcasterConsumer().getGeneratorsInfo(
            "com.bookmap.addons.marketpulse.app.MarketPulse"));
      });
    } else {
      if (connector.isConnectedToProvider(event.addonName)) {
        connector.subscribeToLiveData(event.alias, eventLoop, event.addonName,
                                      event.doesRequireFiltering);
        RpcLogger.info("Successfully connected to " + event.addonName + " " +
                       event.alias);
        return;
      }
      ...
      service.execute(() -> {
        try {
          if (isConnected.get()) {
            connector.subscribeToLiveData(event.alias, eventLoop,
                                          event.addonName,
                                          event.doesRequireFiltering);
            RpcLogger.info("Successfully connected to " + event.addonName +
                           " " + event.alias);
