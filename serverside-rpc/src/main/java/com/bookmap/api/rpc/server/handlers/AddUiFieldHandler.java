...
public class AddUiFieldHandler implements Handler<AddUiField> {
  ...
  private final Map<String, JButton> aliasToApplySettingsButton =
      new HashMap<>();

  private final Map<String, PendingParameterChanges>
      parameterNameToPendingChanges = new ConcurrentHashMap<>();

  public AddUiFieldHandler(Map<String, State> aliasToState,
                           EventLoop eventLoop) {
  ...
  public void handle(AddUiField event) {
      ...
      try {
        ...
        if (!settings.containsParameter(event.name)) {
          settings.addParameter(
              event.name, new RpcSettings.SettingsParameter(
                              event.name, event.fieldType, event.defaultValue));
        }
      } catch (Exception ex) {
	...
	private void addColorField(State state, AddUiField event) {
		...
		var initialColor = (Color) (settings.containsParameter(parameterName) ? settings.getParameter(parameterName).getValue(Color.class) : event.defaultValue);
                                  var colorItem = new ColorsConfigItem(
                                      initialColor, (Color)event.defaultValue,
                                      (color)
                                          -> eventLoop.pushEvent(
                                  ...
                                  state.colorsConfig.add(
                                      colorLabel,
                                      getConstraintsForColors(
                                          0,
                                          getNextRowForComponentInColorsPanel(
                                              state)));
                                  state.colorsConfig.add(
                                      colorItem,
                                      getConstraintsForColors(
                                ...
                                private void addBooleanField(State state,
                                                             AddUiField event) {
                                  ...
                                  panel.add(
                                      ...
                                      getConstraints(
                                          0,
                                          getNextRowForComponentInSettingPanel(
                                              state)));
                                  panel.add(
                                      checkBox,
                                      getConstraints(
                                  ...
                                  var isBoxSelected =
                                      (Boolean)event.defaultValue;
                                  if (state.settings.containsParameter(
                                          event.name)) {
                                    isBoxSelected =
                                        state.settings.getParameter(event.name)
                                            .getValue(Boolean.class);
                                  }

                                  checkBox.setSelected(isBoxSelected);
                                  setNewSettingsValue(
                                      event.alias, state, event.name,
                                      event.fieldType, isBoxSelected, true);
                                ...
                                private void addStringField(State state,
                                                            AddUiField event) {
                                  ...
                                  textField.getDocument().addDocumentListener(
                                      new DocumentListenerAdapter() {
                                        ...
                                        public void onReplace(String newText) {
                                          setNewSettingsValue(
                                              event.alias, state, event.name,
                                              event.fieldType, newText, false);
                                        }
                                      });
                                }

                                private void addNumberField(State state,
                                                            AddUiField event) {
                                  StrategyPanel panel = state.settingsConfig;
                                  RpcSettings settings = state.settings;
                                  String settingParameterName = event.name;

                                  RpcSettings.SettingsParameter parameter =
                                      settings.getParameter(
                                          settingParameterName);
                                  // take initial value either from saved
                                  // settings or from user's specified value
                                  BigDecimal defaultValue =
                                      (BigDecimal)(parameter == null
                                                       ...
                                                       : parameter.getValue(
                                                             BigDecimal.class));
                                  setNewSettingsValue(
                                      event.alias, state, settingParameterName,
                                      event.fieldType, defaultValue, true);

                                  var label = new JLabel(settingParameterName);
                                  var sModel = new SpinnerNumberModel(
                                      defaultValue.doubleValue(),
                                      event.minimum.doubleValue(),
                                      event.maximum.doubleValue(),
                                      event.step.doubleValue());
                                  sModel.addChangeListener(
                                      e
                                      -> setNewSettingsValue(
                                          event.alias, state,
                                          settingParameterName, event.fieldType,
                                          BigDecimal.valueOf(
                                              (double)sModel.getValue()),
                                          false));
                                  panel.add(
                                      label,
                                      getConstraints(
                                ...
                                // taken with small changes from
                                // SimplifiedApiWrapper project
                                ...
                                private void setNewSettingsValue(
                                    String alias, State state,
                                    String parameterName,
                                    AddUiField.FieldType fieldType,
                                    Object newValue, boolean writeChange) {
                                  RpcSettings rpcSettings = state.settings;
                                  RpcSettings.SettingsParameter parameter =
                                      rpcSettings.getParameter(parameterName);
                                  if (parameter == null) {
                                    RpcLogger.info("Unknown parameter " +
                                                   parameterName + " type " +
                                                   fieldType);
                                    if (writeChange) {
                                      RpcLogger.info(
                                          ...
                                          parameterName + ", value - " +
                                          newValue);
                                      rpcSettings.addParameter(
                                          parameterName,
                                          new RpcSettings.SettingsParameter(
                                              parameterName, fieldType,
                                              newValue));
                                      eventLoop.pushEvent(
                                          new OnSettingsParameterChangedEvent(
                                              alias, parameterName, fieldType,
                                              newValue));
                                    } else {
                                      throw new IllegalStateException(
                                          "Trying to set parameter which " +
                                    ...
                                    if (writeChange) {
                                      RpcLogger.info(
                                          "New value will be saved for " +
                                          parameterName +
                                          ", value = " + newValue);
                                      rpcSettings.getParameter(parameterName)
                                          .setValue(newValue);
                                      state.instrumentApi.setSettings(
                                          rpcSettings);
                                      parameterNameToPendingChanges.remove(
                                          parameterName);
                                      eventLoop.pushEvent(
                                          new OnSettingsParameterChangedEvent(
                                              alias, parameterName, fieldType,
                                      ...
                                      RpcLogger.info("Pending changes for " +
                                                     parameterName +
                                                     ", value = " + newValue);
                                      if (parameter.getValue(Object.class)
                                              .equals(newValue)) {
                                        parameterNameToPendingChanges.remove(
                                            parameterName);
                                      } else {
                                        parameterNameToPendingChanges.put(
                                            parameterName,
                                            new PendingParameterChanges(
                                                newValue, fieldType,
                                                parameterName, alias));
                                      }
                                    }
                                  ...
                                  var isButtonEnabled =
                                      !parameterNameToPendingChanges.isEmpty();
                                  var applyButton =
                                      aliasToApplySettingsButton.get(alias);
                                  applyButton.setEnabled(isButtonEnabled);
                                }

                                private static GridBagConstraints
                                getConstraintsForColors(int x, int y) {
                                  GridBagConstraints gbConst =
                                      new GridBagConstraints();
                                  setDefaultConstraints(gbConst, x, y);
                                ...
                                private static void setDefaultConstraints(
                                    GridBagConstraints gbConst, int x, int y) {
                                  gbConst.gridx = x;
                                  gbConst.gridy = y;
                                  gbConst.weightx = 1;
                                  gbConst.weighty = 1;
                                ...
                                // TODO: this is trick by which number of lines
                                // is defined dynamically, try to find more
                                // consistent solution, Maybe move UI to
                                // external class each line has exactly two
                                // components, label and components through
                                // which configuration can be made
                                private int
                                getNextRowForComponentInSettingPanel(
                                    State state) {
                                ...
                                private int getNextRowForComponentInColorsPanel(
                                    State state) {
                                  return state.colorsConfig
                                             .getComponentCount() /
                                      2;
                                }
                                ...
                                private static class PendingParameterChanges {
                                  ...
                                  private PendingParameterChanges(
                                      Object newValue,
                                      AddUiField.FieldType type, String name,
                                      String alias) {
