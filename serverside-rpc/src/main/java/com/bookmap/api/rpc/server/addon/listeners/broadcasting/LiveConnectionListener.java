...
/**
 * The listener that will be notified by BrAPI when the provider's live data
...
public class LiveConnectionListener implements LiveConnectionStatusListener {
  ...
  public LiveConnectionListener(GeneratorInfo generatorInfo,
                                BroadcasterConsumer consumer) {
  ...
  public void reactToStatusChanges(boolean status) {
    ...
    System.out.println("generatorInfo: " + generatorInfo.getSettings() +
                       " filter " + generatorInfo.getFilter());
    List<GeneratorInfo> generatorInfos = consumer.getGeneratorsInfo(
    ...
    executorService.submit(() -> {
      try {
        Thread.sleep(1000);
        System.out.println(
            "generatorInfo after 1 sec: " + generatorInfo.getSettings() +
            " filter " + generatorInfo.getFilter());
  ...
  public boolean isConnect() { return liveConnectionStatus; }
}
